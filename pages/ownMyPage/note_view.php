  ...
  <script>
    $(function() {
      $(document).ready(function() {
        ...
        $(".note-marked").each(function(index) {
          let this_color = "";
          const this_class = $(this).attr('class');
          const uniqueId = "mark_" + index;
        ...
        grouped_array.forEach(function(row) {
          let redObj = row.objects.find(o => o.color === 'red');
          if (redObj) {
            let inner_span = "<span class='note-marked red-mark'>" + redObj.text + "</span>";
            add_html += '<p class="extracted-red-index note-marked red-mark p-2" href="#' + row.id + '" data-unique-id="' + redObj.uniqueId + '">' + inner_span + '</p>' +
              '<div id="' + row.id + '" class="extracted-others" style="display:none;"></div>';
          }
        });
        ...
        extract_btn.on('click', function() {
          ...
          $(".topix-index").eq(2).text("マーク・チェック抽出モード");
        ...
        $(document).on('click', ".extracted-red-index", function() {
          const groupId = $(this).attr('href').substring(1);
          const uniqueId = $(this).data('unique-id');
          const contentDiv = $("#" + groupId);
          ...
          if ($(this).hasClass('spread')) {
            ...
            const groupData = grouped_array.find(g => g.id === groupId);
            const hasGreen = groupData.objects.some(o => o.color === 'green');

            let contentHtml = "";

            if (hasGreen) {
              groupData.objects.filter(o => o.color === 'green').forEach(g => {
                contentHtml += '<p class="extracted-rows p-2" data-unique-id="' + g.uniqueId + '"><span class="note-marked green-mark">' + g.text + '</span></p>' +
                  '<div id="sub_' + g.uniqueId + '" class="extracted-sub-content" style="display:none; padding-left: 1em;"></div>';
              });
            } else {
              contentHtml = extractTextFromDom(uniqueId, "red");
            }

            contentDiv.html(contentHtml).slideDown(300);
          }
        });

        // 抽出した緑見出しマーク押下時トグル機能
        $(document).on('click', ".extracted-rows", function() {
          const uniqueId = $(this).data('unique-id');
          const contentDiv = $("#sub_" + uniqueId);
          ...
          if ($(this).hasClass('spread')) {
            ...
            $(this).addClass('spread');
            const extractedText = extractTextFromDom(uniqueId, "green");
            contentDiv.html(extractedText).slideDown(300);
          }
        });

        function extractTextFromDom(startId, type) {
          let content = "";
          ...
          while (nextNode) {
            if (nextNode.nodeType === 1) { // Element
              const $node = $(nextNode);
              if ($node.hasClass('red-mark')) break;
              if (type === 'green' && $node.hasClass('green-mark')) break;
            }

            if (nextNode.nodeType === 3) { // Text
              content += nextNode.nodeValue;
            } else if (nextNode.nodeType === 1) { // Element
